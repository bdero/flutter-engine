...
std::string CompilerSkSL::compile() {
  ...
  begin_scope();
  statement("  __gl_FragCoord = float4(iFragCoord, 0, 0);");
  statement("  FLT_main();");
...
void CompilerSkSL::emit_header() {
  ...
  statement("");
  statement("float4 __gl_FragCoord;");
  statement("");
}
...
std::string CompilerSkSL::builtin_to_glsl(BuiltIn builtin,
  ...
  switch (builtin) {
    case BuiltInFragCoord:
      // Keep supporting the gl_FragCoord builtin for backwards
      // compatibility/soft transition.
      return "__gl_FragCoord";
    default:
